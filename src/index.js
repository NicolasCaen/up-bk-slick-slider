                ...
                transform: (attributes = {}, innerBlocks = []) => {
                    ...
                    if (Array.isArray(innerBlocks) && innerBlocks.length) {
                        images = innerBlocks
                            ...
                            .map((b) => ({ id: b.attributes.id, url: b.attributes.url, alt: b.attributes.alt || '', caption: b.attributes.caption || '' }))
                            .filter((img) => img.url);
                    } else if (Array.isArray(attributes.images)) {
                        images = attributes.images
                            .map((img) => ({ id: img.id, url: img.url, alt: img.alt || '', caption: img.caption || '' }))
                            .filter((img) => img.url);
                    }
                    return createBlock(
                        metadata.name,
        ...
        to: [
            {
                ...
                transform: ({ slides = [] }) => {
                    const innerBlocks = (slides || [])
                        .filter((s) => s && s.url)
                        .map((s) => createBlock('core/image', { id: s.id, url: s.url, alt: s.alt || '', caption: s.caption || '' }));
